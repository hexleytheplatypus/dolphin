...
namespace WiimoteEmu
{
                      ...
                      u8* const shake_step)
    {
        ...
        unsigned int shake = WiiRemotes[0].emuShake;
...
#define SWING_INTENSITY 2.5  //-uncalibrated(aprox) 0x40-calibrated
    ...
    void Wiimote::UpdateButtonsStatus()
    {
        ...
        for (unsigned i = 0; i < (sizeof(WiiRemotes[m_index].wiimote_keymap) / sizeof(*WiiRemotes[m_index].wiimote_keymap)); i++)
            m_status.buttons.hex |= WiiRemotes[m_index].wiimote_keymap[i].value ? WiiRemotes[m_index].wiimote_keymap[i].dolphinButton : 0;
    }
    ...
    void Wiimote::GetIRData(u8* const data, bool use_accel)
    {
        ...
        //xx = (.5 - (WiiRemotes[0].dx / 1023) ) * 1.2;
       // yy = ((WiiRemotes[0].dy / 767) - .5) * 1.2;
        //zz = 0 ;
...
//        WiiRemotes[0].dy = -1;
    ...
    bool Wiimote::CheckForButtonPress()
    {
        ...
        for (unsigned i = 0; i < (sizeof(WiiRemotes[m_index].wiimote_keymap) / sizeof(*WiiRemotes[m_index].wiimote_keymap)); i++)
                  buttons |= WiiRemotes[m_index].wiimote_keymap[i].value ? WiiRemotes[m_index].wiimote_keymap[i].dolphinButton : 0;
